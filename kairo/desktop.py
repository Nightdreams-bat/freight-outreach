...
This is what a plain double-click / `python -m kairo` does. Flask runs on a
...
def _start_server():
    ...
    from kairo.web.app import _pick_port, create_app
...
def _maybe_create_shortcuts():
    ...
    if marker.exists():
        return
    try:
        from kairo.shortcut import create_shortcuts
...
def run_desktop():
    ...
    if not _open_app_mode(url):
        webbrowser.open(url)
    print(f"Kairo is running at {url}  -  close this window to stop it.")
    try:
        thread.join()
